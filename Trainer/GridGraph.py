...
class GridGraph(object):
    def __init__(self, gridParameters,max_step=100,twopin_combo=None,net_pair=[1,2,3]):
        ...
        self.route = []
        self.reward = 0.0
        self.instantreward = 0.0;  self.instantrewardcombo = [];  self.best_reward = 0.0
    ...
    def step(self, action):  # used for DRL
        ...
        # print(self.current_state,"current")
        # print(self.init_state,"init state")
        # print(self.goal_state,"goal_state")
        # print(self.state2obsv(),"obs")
        """
                x, y, lay
    ...
    def reset(self, max_episodes = None):
        '''
        ...
        if self.twopin_pt >= len(self.twopin_combo):
            ...
            if sum([1 for item in self.instantrewardcombo if item > 0]) > self.posTwoPinNum:
                ...
                if self.reward>self.best_reward:
                    ...
                    is_best = 1
            if max_episodes:
                if self.posTwoPinNum==len(self.twopin_combo)-1:
                     print(f"get totpinnum-1 at episode{self.episode}")
                print(f'\rNew loop!    Episode: {self.episode+1}/{max_episodes}  \
Reward:  {self.reward}   Positive two pin num: {self.posTwoPinNum}/{len(self.twopin_combo)}',end='')
            else: 
                print(f'\rNew loop!    Episode: {self.episode+1}  \
Reward:  {self.reward}   Positive two pin num: {self.posTwoPinNum}/{len(self.twopin_combo)}',end='')
            self.route_combo = []
            self.reward = 0.0
