...
class PrioritizedReplayBuffer(ReplayBuffer):
    ...
    def __init__(self, obs_dim: int, max_size: int=50000,  batch_size: int = 32, 
        ...
        while tree_capacity < self.max_size:
            tree_capacity *= 2
        #sum tree for prior  #* we can find sum in any segment in logN time (instead of N)
        self.sum_tree = SumSegmentTree(tree_capacity)  
        # min tree for min prior to get max weight #* we can find min in any segment in logN time 
        self.min_tree = MinSegmentTree(tree_capacity)  
    def store( self, obs: np.ndarray, act: int, 
        rew: float, next_obs: np.ndarray, done: bool
    ):
...
class DQN_Agent(): 
  def __init__(self,  gridgraph, self_play_episode_num=20):
    ...
    self.optim = tc.optim.Adam(self.dqn.parameters(),lr=0.0001)  #! lr has changed
